...
* *`style` is allowed, `class` is not.* DevOps inlines colours and alignment on
  spans and table cells, and dropping `style` mangles pasted content; DOMPurify
  parses the declaration list and removes anything that can fetch or execute.
  `class` is different — our own stylesheet gives class names meaning, and a
  comment author should not be able to borrow UI chrome.
