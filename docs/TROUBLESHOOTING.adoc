...
|"Request Header Fields Too Large" — the `Cookie` header exceeded Node's 16 KB `maxHeaderSize`. All localhost ports share one cookie jar, so cookies from every other dev app you've run accumulate on `localhost`, and the NextAuth session cookie is itself large because it carries the Azure AD access, refresh and id tokens (chunked as `next-auth.session-token.0`, `.1`, …). Fix either side: clear cookies for `localhost` (Chrome: DevTools → Application → Cookies → `http://localhost:3000` → Clear all, or visit `chrome://settings/content/all?searchSubpage=localhost`), or start the server with a bigger limit — `NODE_OPTIONS=--max-http-header-size=65536 bunx next dev --port 3000`. Confirm which it is with `curl -s -o /dev/null -w "%{http_code}" http://localhost:3000/` — a plain curl sends no cookies, so it returns 200 while the browser gets 431.

|**Sign-in redirect fails / lands back on the login page in local dev**
|`NEXTAUTH_URL` must match the port the dev server actually binds. `bun run dev` binds **3102** (see `package.json`), while `.env.example` suggests 3000 — pick one and make the Azure AD app registration's redirect URI agree: `http://localhost:<port>/api/auth/callback/azure-ad`. A mismatch shows up as `AADSTS50011`.
|===
