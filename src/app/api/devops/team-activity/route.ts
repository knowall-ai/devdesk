...
export async function GET(request: Request) {
  try {
    ...
    // Filter tickets by team member if specified
    const teamMemberIds = new Set(allMembers.map((m) => m.id));
    const relevantTickets = tickets.filter((ticket) => {
      // Check if assigned to a team member
      const isAssignedToTeam = ticket.assignee && teamMemberIds.has(ticket.assignee.id);

      // If member filter is specified, only count activities from that member
      if (memberFilter && memberFilter !== 'all') {
        return ticket.assignee?.id === memberFilter;
      }
